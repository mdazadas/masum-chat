...
- **Global scope**: `F:/Websites/Masum Chats/.gemini/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `F:/Websites/Masum Chats/.gemini/notes/`)
...
2. Glob `F:/Websites/Masum Chats/.gemini/notes/*.md` (if directory exists) — global notes
...
Global (F:/Websites/Masum Chats/.gemini/notes/):
...
2. **No `.planning/`**: Falls back to global `F:/Websites/Masum Chats/.gemini/notes/` — works in any directory
